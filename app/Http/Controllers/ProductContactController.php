...
class ProductContactController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::user()->role == 'Admin') {
            ...
            $data = $query->where('supplier_id', Auth::user()->id)->orderBy('id', 'desc')->get();
        }

        return response()->view('products.customerqueries', compact('data', 'query'), 200);
    }
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'pro_model_name' => 'required|string',
        ]);

        $productContact = new ProductContact();
        $productContact->customer_id = $request->input('customer_id');
        $productContact->supplier_name = $request->input('supplier_name');
        $productContact->customer_name = $request->input('customer_name');
        $productContact->pro_name = $request->input('pro_name');
        $productContact->pro_model_name = $request->input('pro_model_name');
        $productContact->save();

        return redirect()->back()->with('success', 'Product contact information stored successfully!');
    }
}
