...
use App\Models\VendorBankCardDetail;
...
class VendorsController extends Controller
{
    ...
    public function show($id)
    {
        ...
        // $bdata = VendorBankDetail::where('vendor_id', '=', $id)->get();
        //     dd($bdata);
        //     $edit = vendorProfile::all();
        // dd($edit);
        $edit = vendorProfile::with('user')->where('vendor_id', '=', $id)->get();
        $bankDetail = VendorBankDetail::where('vendor_id', '=', $id)->get();
        $vendordocument = VendorDocument::where('vendor_id', '=', $id)->get();
        $services = Services::where('vendor_id', '=', $id)->get();
        $porfolie = VendorPortfolio::where('vendor_id', '=', $id)->get();
        $bdata = VendorBankCardDetail::where('vendor_id', $id)->get();
        $vendors = user::find($id);
        // dd($edit, $bankDetail, $vendordocument,$services,$porfolie,$bdata);
        return view('sellers.vendorview', compact('edit', 'bankDetail', 'vendordocument','services','porfolie','bdata','vendors'));
    }
    ...
    //     $services = Services::where('vendor_id', '=', $id)->get();
    //     $porfolie = VendorPortfolio::where('vendor_id', '=', $id)->get();
    //     return view('sellers.vendorview', compact('edit', 'bankDetail', 'vendordocument','services','porfolie'));
    // }
    ...
    //     // $status = array('0' => 'Active', '1' => 'In Active');
    //     // // echo "hello";
    //     // return view('sellers.vendorview', compact('vendors', 'status'));
    // }
    ...
    public function vendorProfile($id)
    {
        ...
        $bankDetail = VendorBankDetail::with('vendorProfile')->where('vendor_id', '=', $id)->get();
        $vendordocument = VendorDocument::with('vendor_id')->where('vendor_id', '=', $id)->get();
        $data = VendorBankDetail::with('vendorProfile')->where('vendor_id', '=', $id)->get();
        $bankCard = VendorBankCardDetail::where('vendor_id', $id)->get();
        // dd($edit);
        if (!$edit) {
        ...
        if ($edit) {
            ...
            return view('sellers.vendorprofile', compact('edit', 'accepted_payment_type', 'pay','bankCard','bankDetail','vendordocument','data'));
        }
    }
    ...
    public function verifiedSeller($id)
    {
        ...
        $data = VendorBankDetail::with('vendorProfile')->where('vendor_id', '=', $id)->get();
        $bankCard = VendorBankCardDetail::where('vendor_id', $id)->get();
        // dd($edit, $bankDetail, $vendordocument, $data);
        // dd($edit);
        ...
        return view('sellers.vendorprofile', compact('edit', 'bankDetail', 'vendordocument','bankCard'));
    }
    ...
    public function sellerCardSave(Request $request, $id)
    {
        // dd($request->all());
        $this->validate($request, [
            'vendor_profile_id' => 'required',
            ...
            // 'valid_date' => 'required|date_format:m/Y',
            // 'card_front' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048',
            // 'card_back' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048',          
        ]);
        $edit = new VendorBankCardDetail;

        $edit->vendor_profile_id = $request->vendor_profile_id;
        $edit->vendor_id = $request->vendor_id;
        $edit->card_holder_name = $request->card_holder_name;
        $edit->card_number = $request->card_number;
        $edit->cvv = $request->cvv;
        $edit->valid_date = $request->valid_date;
        $edit->card_type = $request->card_type;
        // $cardFrontPath = $request->file('card_front')->store('public/images');
        // $cardBackPath = $request->file('card_back')->store('public/images');
    
        // // Update $bdata with the image paths
        // $bdata->card_front = str_replace('public/', 'storage/', $cardFrontPath);
        // $bdata->card_back = str_replace('public/', 'storage/', $cardBackPath);
    
        // Save $bdata to database or do whatever you want
        // echo "<pre>";
        // print_r($bdata);
        // echo "</pre>";
        $edit->save();
        ...
        return view('sellers.verifiedseller', compact('edit'));
        // return redirect()->route('sellerCardShow/{id}');
    }

    public function sellerCardShow(Request $request, $id)
    {
        $edit = vendorProfile::with('user')->where('vendor_id', '=', $id)->first();
        $bankDetail = VendorBankDetail::with('vendor_profile')->where('vendor_id', '=', $id)->get();
        $vendordocument = VendorDocument::with('vendor_profile')->where('vendor_id', '=', $id)->get();
        $bankCardData = VendorBankDetail::all();
        // dd($edit);
        if (!$edit) {
            $edit = new VendorProfile();
            $edit->vendor_id = $id;
        ...
        return view('sellers.verifiedseller', compact('edit', 'bankDetail', 'vendordocument','data'));
    }
